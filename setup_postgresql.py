...
def create_pg_db(user, password, database, database_path):
    """
        Initialize PostgreSQL Database, add database user und create the  Database.
    """
    os.makedirs(database_path)
    set_pg_permission(database_path)
    # initialize a new postgres database
    subprocess.call("su - postgres -c '%s --auth=trust --pgdata=%s'" %
                    (os.path.join(PG_BIN, 'initdb'), database_path), shell=True)
    ...
    subprocess.call( """su - postgres -c "psql --command \\"CREATE ROLE launch LOGIN CREATEDB PASSWORD 'pwd'\\";"
                    """, shell=True )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Initializing a complete Database.')

    parser.add_argument("--dbuser", required=True,
                    help="Username of the Database Administrator.")

    parser.add_argument("--dbpassword", required=True,
                    help="Password of the Database Administrator.")
    ...
    """
        Initialize the Database + adding an Admin user.
